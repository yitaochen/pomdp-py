...
import pomdp_py
import random
from pomdp_problems.tiger.tiger_problem import TigerState, TigerObservation, TigerAction, TigerProblem
from pomdp_problems.tiger.interactive_tiger_problem import ITigerState, IBelief, ITigerProblem
...
def qvalue(b, a, S, A, Z, T, O, R, gamma, horizon=1):
    ...
    if horizon > 1:
        for o in Z:
            ...
            if prob_o > 0:
                next_b = i_belief_update(b, a, o, A, Z, T, O)
...
def expected_reward(b, R, a, T=None):
    ...
    for s in b:
        if T is not None:
            for sp in b:
                r += b[s] * R.sample(s.state(), a, sp.state()) * T.probability(sp.state(), s.state(), a)
        else:
            r += b[s] * R.sample(s.state(), a, None)
    return r
...
def belief_observation_model(o, b, a, T, O):
    ...
    for s in b:
        for sp in b:
            trans_prob = T.probability(sp.state(), s.state(), a)
            obsrv_prob = O.probability(o, sp.state(), a)
...
def belief_update(b, a, o, T, O):
    ...
    for s in next_b:
        # normalize
        next_b[s] = round(next_b[s]/total_prob, 2)
    return next_b

def i_belief_update(b, ai, oi, A, Z, T, O):
    """Returns the updated belief of `b` given 
    actions `ai`, `aj` and observations `oi`, `oj`
    """
    next_b = dict()
    total_prob = 0.0
    for is_curr in b:
        for aj in A:
            prob = O.probability(oi, is_curr.state(), ai)
            for oj in Z:
                # print(belief_update(is_curr.belief(), aj, oj, T, O))
                # tmpBelief = belief_update(is_curr.belief(), aj, oj, T, O)
                # print("%s, Action j: %s, Obeservation j: %s" % (tmpBelief, aj, oj))
                is_next = ITigerState(is_curr.state(), belief_update(is_curr.belief(), aj, oj, T, O))
                trans_prob = T.probability(is_curr.state(), is_curr.state(), ai) * O.probability(oj, is_curr.state(), aj) * b[is_curr]
                prob *= trans_prob
                tmp_prob = next_b.get(is_next, 0)
                if tmp_prob + prob > 1e-9:
                    next_b[is_next] = tmp_prob + prob
                    total_prob += prob
    
    for is_next in next_b:
        # normalize
        next_b[is_next] /= total_prob
    
    # print(sum([prob for istate, prob in next_b.items()]))
    
    return IBelief(next_b)
...
def _create_case(noise=0.15, init_state="tiger-left"):
    ...
    tiger = ITigerProblem(0.15,  # observation noise
...
def _test_planning():
    ...
    for step in range(10):
        ...
        # print(b)
        b = i_belief_update(b, a_best, o, A, Z, T, O)
